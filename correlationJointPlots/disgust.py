...
# Regression Plot 1: Disgust vs positive
r, _ = stats.pearsonr(df_pos['disgust_n'], df_pos['positive_n'])
a0 = sns.jointplot(x="disgust_n", y="positive_n", data=df_pos, kind='reg', fit_reg=True, ci=90, color="darkgreen")
a0.ax_joint.annotate(f'$\\rho = {r:.3f}$',
                     xy=(0.1, 0.9), xycoords='axes fraction',
                     ha='left', va='center',
                     bbox={'boxstyle': 'round', 'fc': 'powderblue', 'ec': 'navy'})
a0.fig.suptitle("Disgust vs Positive (Regression)", fontsize=16)
a0.fig.tight_layout()
a0.fig.subplots_adjust(top=0.95)
a0.fig.savefig(f"{output_dir}/disgust_vs_positive_reg.png", bbox_inches='tight')
plt.close(a0.fig)

# KDE Plot 1
kde1 = sns.jointplot(x="disgust_n", y="positive_n", data=df_pos, kind='kde', shade=True, cmap="GnBu")
kde1.fig.suptitle("Disgust vs Positive (KDE)", fontsize=16)
kde1.fig.tight_layout()
kde1.fig.subplots_adjust(top=0.95)
kde1.fig.savefig(f"{output_dir}/disgust_vs_positive_kde.png", bbox_inches='tight')
plt.close(kde1.fig)
